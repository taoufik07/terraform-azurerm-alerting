...
data "azurecaf_name" "activity_log_alert" {
  for_each = var.activity_log_alerts

  name          = var.stack
  resource_type = "azurerm_resource_group"
  prefixes      = compact([local.name_prefix, var.use_caf_naming ? "activity_log_alert" : ""])
...
# Custom naming until managed by CAF provider
data "azurecaf_name" "metric_alert" {
  for_each = var.metric_alerts

  name          = var.stack
  resource_type = "azurerm_resource_group"
  prefixes      = compact([local.name_prefix, var.use_caf_naming ? "metric_alert" : ""])
